  ...
  return (
    <div className="pb-4">
      ...
      <div className="px-4 pt-4 pb-3">
        <h2 className="font-headline font-bold text-xl">Following</h2>
      </div>
          ...
          {(['artists', 'venues'] as Tab[]).map(tab => (
            <button
              ...
              className={`
                ...
                ${activeTab === tab
                  ? 'bg-primary-container text-white shadow-[0_0_12px_rgba(255,107,53,0.2)]'
        ...
        <div className="px-4">
          {INITIAL_ARTISTS.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-on-surface-variant text-sm">No artists followed yet</p>
              <p className="text-outline text-xs mt-1">Discover artists to follow</p>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              {INITIAL_ARTISTS.map(artist => {
                ...
                return (
                  <div
                    key={artist.id}
                    className="bg-surface-container border border-white/5 rounded-2xl p-3.5 flex items-center gap-3"
                  >
                    <div
                      className="w-12 h-12 rounded-full flex items-center justify-center text-white font-headline font-bold text-sm shrink-0 cursor-pointer"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <Badge variant="default" className="text-xs">{artist.genre}</Badge>
                        <span className="text-outline text-xs">{formatCompactNumber(artist.followers)} followers</span>
                      </div>
                    </div>
                    <Button
        ...
        <div className="px-4">
          {INITIAL_VENUES.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-on-surface-variant text-sm">No venues followed yet</p>
              <p className="text-outline text-xs mt-1">Explore venues to follow</p>
            </div>
          ) : (
            <div className="flex flex-col gap-2">
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <p className="text-outline text-xs mt-0.5">{venue.type} &middot; {venue.location}</p>
                    </div>
                    <Button
                      variant={isFollowing ? 'teal' : 'outline'}
                      size="sm"
