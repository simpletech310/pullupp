...
function TypingIndicator() {
  return (
    <div className="flex items-center gap-1 px-4 py-2.5 bg-surface-container rounded-2xl rounded-bl-md w-fit">
...
export default function ChatPage() {
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-bg">
      ...
      <header className="sticky top-0 z-40 bg-bg/80 backdrop-blur-xl border-b border-white/5">
        <div className="flex items-center gap-3 h-14 px-4">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-headline font-bold text-sm truncate">{contact.name}</h1>
              ...
              >
                <div
                  className={`max-w-[80%] px-4 py-2.5 ${
                    msg.sent
                      ? 'bg-primary-container/90 text-white rounded-2xl rounded-br-md'
                      : 'glass-card rounded-2xl border border-white/5 text-on-surface rounded-bl-md'
                  }`}
                >
                  <p className="text-sm leading-relaxed">{msg.text}</p>
                  <p className={`text-xs mt-1 text-right ${msg.sent ? 'text-white/60' : 'text-outline'}`}>
                    {msg.timestamp}
                  </p>
                </div>
              </div>
      ...
      {/* Input bar */}
      <div className="sticky bottom-0 bg-bg border-t border-white/5 px-4 py-3 pb-[calc(0.75rem+env(safe-area-inset-bottom))]">
        <div className="flex items-center gap-2">
          <input
            ...
            className="flex-1 bg-surface-container border border-white/5 rounded-full px-4 py-2.5 text-sm text-on-surface placeholder:text-outline focus:outline-none focus:border-primary-container focus:ring-1 focus:ring-primary-container/30 transition-colors font-body"
          />
          <button
            onClick={handleSend}
            disabled={!inputValue.trim()}
            className="w-10 h-10 rounded-full bg-primary-container text-white flex items-center justify-center shrink-0 hover:bg-orange-light transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
          >
