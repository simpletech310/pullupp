...
function MenuItem({ icon, label, onClick, danger }: MenuItemProps) {
  return (
    <button
      ...
      `}
    >
      <span className={`w-5 h-5 flex items-center justify-center ${danger ? 'text-error' : 'text-text-secondary'}`}>
        {icon}
      </span>
...
function MenuSection({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mt-6">
      <h3 className="px-4 text-xs font-semibold text-text-muted uppercase tracking-wider mb-1">{title}</h3>
...
export default function ProfilePage() {
  ...
  return (
    <div className="pb-8 animate-fade-in">
      ...
      <div className="flex flex-col items-center pt-8 pb-6 px-4">
        ...
        <div className="relative mb-4">
          {profile?.avatar_url ? (
            ...
            <div
              className="w-24 h-24 rounded-full flex items-center justify-center text-2xl font-display font-bold text-white border-2 border-border"
              style={{
          ...
          <div
            className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-surface border-2 border-border flex items-center justify-center cursor-pointer"
            onClick={() => router.push('/settings')}
          >
            <svg fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-3.5 h-3.5 text-text-secondary">
      ...
      {/* Stats Row */}
      <div className="flex items-center justify-center gap-0 px-4 pb-6">
        <div className="flex-1 text-center">
          <div className="font-display font-bold text-lg text-text-primary">12</div>
          <div className="text-xs text-text-secondary">Events</div>
        </div>
        <div className="w-px h-8 bg-border" />
        <div className="flex-1 text-center">
          <div className="font-display font-bold text-lg text-text-primary">48</div>
          <div className="text-xs text-text-secondary">Following</div>
        </div>
        <div className="w-px h-8 bg-border" />
        <div className="flex-1 text-center">
          <div className="font-display font-bold text-lg text-text-primary">5</div>
          <div className="text-xs text-text-secondary">Tickets</div>
        </div>
      </div>
        ...
        <div className="px-4 pb-6">
          <div className="flex flex-wrap gap-2">
            {profile.interests.map((interest) => (
              <span
                key={interest}
                className="px-3 py-1.5 bg-surface border border-border rounded-full text-xs font-medium text-text-secondary"
              >
                {interest}
              </span>
            ))}
