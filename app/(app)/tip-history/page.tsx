  ...
  return (
    <div className="pb-4">
      ...
      <div className="px-4 pt-4 pb-3 flex items-center gap-3">
        ...
        <h2 className="font-display font-bold text-xl">Tip History</h2>
      </div>

      {/* Total Stat */}
      <div className="px-4 mb-4">
        <div className="bg-surface border border-border rounded-2xl p-4 text-center">
          <p className="text-text-secondary text-xs uppercase tracking-wide mb-1">Total Tips Sent</p>
          <p className="font-display font-bold text-3xl text-orange">{formatCurrency(TOTAL_TIPS)}</p>
          <p className="text-text-muted text-xs mt-1">{MOCK_TIPS.length} tips total</p>
        </div>
      </div>
        ...
        <div className="flex flex-col gap-2">
          {MOCK_TIPS.map(tip => {
            ...
            return (
              <div
                key={tip.id}
                className="bg-surface border border-border rounded-2xl p-3.5 flex items-center gap-3 cursor-pointer hover:border-border-light transition-colors"
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <span className="text-text-muted text-xs truncate">{tip.context}</span>
                  </div>
                </div>

                {/* Amount + Date */}
                <div className="text-right shrink-0">
                  <p className="text-orange font-semibold text-sm">{formatCurrency(tip.amount)}</p>
                  <p className="text-text-muted text-[11px] mt-0.5">{formatTimeAgo(tip.date)}</p>
                </div>
              </div>
            );
          })}
